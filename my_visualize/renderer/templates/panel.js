...
function showDataPanel(node) {
  ...
  if (!panel) {
    console.error('[my_visualize] data-panel 엘리먼트를 찾을 수 없습니다:', dataPanelId);
    return;
  }
  ...
  try {
    ...
    console.log('[my_visualize] 패널 렌더링 완료 | 노드:', node.name, '| shape:', JSON.stringify(node.output_shape));

  } catch(err) {
    console.error('[my_visualize] ❌ 패널 렌더링 오류:', err);
    panel.innerHTML = `
      <div style="color:#f87171; padding:16px; font-size:12px;">
        <strong>패널 오류:</strong><br>
        <span style="color:#94a3b8;">${err.message || String(err)}</span>
      </div>`;
  }
}
